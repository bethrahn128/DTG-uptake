---
title: "SA256 DTG uptake <br> IWHOD2024_Abstract_Fig1"
subtitle: " "
...
dtg1 <- tblBAS %>% filter(age_cat3 == "[20,30)" | age_cat3 =="[30,40)") %>%
  ...
  group_by(ctry, country, sex, qtr, recart_d_qtr, recommend_d_qtr,dtg_recomend_d, recommend_date,  min_dtg_date) %>%
  summarise(
    numer = sum(dtg_first),
    denom = n()) %>%
  mutate(line = "dtg") %>%
...
#ggsave("output/Abstract/DTG_initiation_by_ctry_sex_20-39_by_month.png", width = 7, height =8)  
...
Panel A) Cumulative incidence (1-KM plots) of intiating DTG by country and sex over calendar year. 
(aggregated quarterly)
 ...
 geom_line(stat="identity", size= 1.5, linetype = 1, show.legend = TRUE) +
  ...
  theme(plot.title = element_text(size = 12, hjust = 0.5, vjust =0),
       ...
       #legend.box.background = element_rect(color = "black"),
        ...
        strip.background = element_blank(),
        strip.text.x = element_text(size=13),
        axis.title.y = element_text(margin = margin(t = 1, r = 1, b = 1, l = 1),size=12), #pad x axis label,
...
plotA_row1 <- plotA_LSO + plotA_MWI
#plotA_row1

plotA_row2 <- plotA_MOZ  + plotA_ZAF
#plotA_row2

plotA_row3 <- plotA_ZMB + plotA_ZWE
#plotA_row3
...
plotA_v1 <- plotA_row1 / plotA_row2 / plotA_row3 + plot_layout(guides = "collect") & theme(legend.position = "bottom", legend.justification = "left")
plotA_v1
```


```{r}
ggsave("output/IWHOD2024_Abstract/PanelA_ctry_sex20-39.png", width =4.5, height =6)
```
...
Panel B) Cumulative incidence of DTG switch: By country by sex
...
fit_LSO <- survfit2(Surv(time_t2, dtg_comp_b) ~ sex3.factor, data = FU_time_temp, etype = dtg_switch_txt_crr2b.factor)
...
plotB_LSO <- survfit2(Surv(time_t2, dtg_switch_txt_crr2b.factor) ~ sex3.factor, data = FU_time_temp) %>%
...
plotB_MWI <- survfit2(Surv(time_t2, dtg_switch_txt_crr2b.factor) ~ sex3.factor, data = FU_time_temp) %>%
   ggcuminc(outcome = c("Switch to dolutegravir", "Competing risks"), 
           linetype_aes = FALSE, 
           theme = theme_ggsurvfit_default(), 
           size = 1.5) +
...
plotB_MOZ <- survfit2(Surv(time_t2, dtg_switch_txt_crr2b.factor) ~ sex3.factor, data = FU_time_temp) %>%
   ggcuminc(outcome = c("Switch to dolutegravir", "Competing risks"), 
           linetype_aes = FALSE, 
           theme = theme_ggsurvfit_default(), 
           size = 1.5) +
...
plotB_ZAF <- survfit2(Surv(time_t2, dtg_switch_txt_crr2b.factor) ~ sex3.factor, data = FU_time_temp) %>%
    ggcuminc(outcome = c("Switch to dolutegravir", "Competing risks"), 
           linetype_aes = FALSE, 
           theme = theme_ggsurvfit_default(), 
           size = 1.5) +
...
plotB_ZMB <- survfit2(Surv(time_t2, dtg_switch_txt_crr2b.factor) ~ sex3.factor, data = FU_time_temp) %>%
   ggcuminc(outcome = c("Switch to dolutegravir", "Competing risks"), 
           linetype_aes = FALSE, 
           theme = theme_ggsurvfit_default(), 
           size = 1.5) +
...
plotB_ZWE <- survfit2(Surv(time_t2, dtg_switch_txt_crr2b.factor) ~ sex3.factor, data = FU_time_temp) %>%
  ggcuminc(outcome = c("Switch to dolutegravir", "Competing risks"), 
           linetype_aes = FALSE, 
           theme = theme_ggsurvfit_default(), 
           size = 1.5) +
...
plotB_row1 <- plotB_LSO  + plotB_MWI 
plotB_row1

plotB_row2 <- plotB_MOZ + plotB_ZAF 
plotB_row2

plotB_row3 <- plotB_ZMB + plotB_ZWE 
plotB_row3
...
```{r echo=FALSE, fig.height=6, fig.width=6}
plotB_v1 <- plotB_row1 / plotB_row2 /  plotB_row3 + plot_layout(guides = "collect") & theme(legend.position = "bottom", legend.justification = "left")
plotB_v1
```
...
ggsave("output/IWHOD2024_Abstract/PanelB_ctry_sex20-39.png", width =4.5, height =6)
```
...
combo <- 
  ...
  plot_layout(nrow = 3, widths= c(6, 6, 1, 6, 6), guides = "collect") & theme(legend.position = "bottom", legend.justification = "left")
...
```{r}
ggsave("output/IWHOD2024_Abstract/Fig1_ctry_sex20-39_no_panel_txt.png", width = 8.5, height =9)
```
...
Fig1 <- 
  ...
  (plotA_LSO + plotA_MWI + plot_spacer() + plotB_LSO + plotB_MWI + plot_layout(ncol = 5, widths= c(6, 6, 1, 6, 6))) / 
  
  
  (plotA_MOZ + plotA_ZAF + plot_spacer() + plotB_MOZ + plotB_ZAF + plot_layout(ncol = 5, widths= c(6, 6, 1, 6, 6))) / 
  
  
  (plotA_ZMB + plotA_ZWE + plot_spacer() + plotB_ZMB + plotB_ZWE + plot_layout(ncol = 5, widths= c(6, 6, 1, 6, 6))) /  
...
Fig1b <- ggdraw(add_sub(Fig1, color = "black",  size = 12.5, vpadding = grid::unit(0.5,"lines"), x = 0, y = 0.4, hjust = 0, 
  "Panel A: Percentage initiating dolutegravir-based ART among those initiating any ART, aggregated \ntri-monthly over calendar year.\nPanel B: Cumulative incidence of switching to dolutegravir-based ART and competing risk events for \nswitching to dolutegravir-based ART (i.e., death, transfer out and loss to follow-up) over calendar year.\nVertical dashed lines show the date of national dolutegravir adoption.                     "))
...
```{r echo=FALSE, fig.height=9.5, fig.width=8.5}
Fig1b
...
```{r}
ggsave("output/IWHOD2024_Abstract/Fig1_ctry_sex20-39_with_panel_txt.png", width = 8.5, height =9.5)
```
