---
title: "IeDEA SA256 DTG uptake <br> 020 Initiating cohort Outcomes"
subtitle: " "
...
tblBAS <- tblBAS %>%
  select(-birth_d, -aids_d, -aids_d_a, 
         ...
         -l_alive_d, -l_alive_d_a, -dtg_status, -dtg_status_txt, -dtg_switch, -dtg_outcome, -dtg_min_program) 
...
Programs with patients starting ART >= 24 months after national DTG adoption == 1


```{r}
maxclose <- tblBAS %>% select(country, program, max_close_d, dtg_recomend_d, FU_adoption_24m) %>%
  arrange(max_close_d, dtg_recomend_d, country, program) %>%
  group_by(country, program,  max_close_d, dtg_recomend_d, FU_adoption_24m) %>%
  mutate(Npatients = n()) %>%
  distinct() %>%
...
```{r}
maxclose2 <- maxclose %>% select(country, program, max_close_d, dtg_recomend_d, FU_adoption_24m) %>% mutate(keep =1) %>% mutate(tempname = paste0(program, max_close_d, sep = ""))

listprograms <- tblBAS %>% select(country, program, max_close_d, dtg_recomend_d) %>%
 mutate(tempname = paste0(program, max_close_d, sep = "")) %>%
  group_by(tempname) %>%
...
write_fst(exclude, "data_temp/tblBAS_initiation_programsexcluded_ART24m_v2.fst")
```
...
### 3. Ensure all patients have >=15 months of potential follow-up since ART initiation
...
Viral load: recommended at 6 months & 12 months and then yearly after.
...
rm(listprograms, exclude, maxclose, maxclose2)
...
```{r}
write_fst(tblBAS, "data_temp/tblBAS_initiation_v2_outcome_LTFU_who.fst")
```
...
write_fst(Initiation_listprograms, "data_temp/tblBAS_initiation_outcomes_listprograms_v2.fst")
...
#ggsave("output/Cohort_initiation_outcomes/SA256_DTG_status_country_cohort1_v2.png", width = 8, height =4)
...
ggsave("output/Cohort_initiation_outcomes/DTG_initiation_by_country_sex.png", width = 7, height =5)
```
...
ggsave("output/Cohort_initiation_outcomes/DTG_initiation_by_sex_age.png", width = 7, height =6)
```
...
ggsave("output/Cohort_initiation_outcomes/DTG_vs_other_country.png", width = 7, height =6) 
...
ggsave("output/Cohort_initiation_outcomes/DTG_vs_other_sex_age.png", width = 7, height =5)
...
ggsave("output/Cohort_initiation_outcomes/DTG_by_ctry_sex_by_month.png", width = 7, height =8)  
...
ggsave("output/Cohort_initiation_outcomes/DTG_by_sex_by_month.png", width = 7, height =8)  
...
ggsave("output/Cohort_initiation_outcomes/DTG_by_sex_over_time_period.png", width = 7, height =8)  
...
ggsave("output/Cohort_initiation_outcomes/DTG_ZAF_by_sex_over_time_period.png", width = 5, height =8)  
...
```{r include=FALSE}
fit1_dtg <- survfit2(Surv(time, dtg_comp_b) ~ dtg_first, data = FU_time, etype = dtg_first_txt_crr1b.factor)
ggcompetingrisks(fit = fit1_dtg) +
...
p01 <-ggcompetingrisks(fit = fit1_dtg,
                 xlab = "Years since ART initiation", 
                  ylab = "Cumulative incidence",
                 title = "Outcome among adults initiating ART") +
...
ggsave("output/Cohort_initiation_outcomes/Fig4_Initiation_adults_Cumul_incid_all_outcomes.png", width = 5, height =6)
...
fit1_dtg_sex <- survfit2(Surv(time, dtg_comp_b) ~ dtg_first + sex, data = FU_time, etype = dtg_first_txt_crr1b.factor)
ggcompetingrisks(fit = fit1_dtg_sex) +
...
p03 <-ggcompetingrisks(fit = fit1_dtg_sex,
                 xlab = "Years since ART initiation", 
                  ylab = "Cumulative incidence",
                 title = "Outcome among adults initiating ART") +
     scale_x_continuous(breaks = c(0, 365,  730, 1095, 1460 ), limits = c(0,1460), 
 ...
 facet_wrap(~strata, ncol = 2, labeller = labeller(strata = 
      c("dtg_first=0, sex=Female" = "Initiated other ART, female", 
        "dtg_first=0, sex=Male  " = "initiated other ART, male", 
...
ggsave("output/Cohort_initiation_outcomes/Fig4_Initiation_sex_Cumul_incid_all_outcomes.png", width = 6, height =6)
...
fit1_ctry_dtg <- survfit2(Surv(time, dtg_comp_b) ~ ctry + dtg_first , data = FU_time, etype = dtg_first_txt_crr1b.factor)
...
p05 <-ggcompetingrisks(fit = fit1_ctry_dtg,
                 xlab = "Years since ART initiation", 
                  ylab = "Cumulative incidence (%)",
                 title = "",
                 subtitle ="(A) Initiated other ART          (B) Initiated DTG ART") +
 ...
 scale_fill_manual("",
  ...
  name = "Outcome event") +
 facet_wrap(~strata, ncol = 2, labeller = labeller(strata = 
      c("ctry=Lesotho, dtg_first=0" = "Lesotho", 
        "ctry=Lesotho, dtg_first=1" = "Lesotho", 
        
         "ctry=Malawi, dtg_first=0" = "Malawi", 
        "ctry=Malawi, dtg_first=1" = "Malawi", 
       
        "ctry=Mozambique, dtg_first=0" = "Mozambique", 
        "ctry=Mozambique, dtg_first=1" = "Mozambique", 
                
        "ctry=South Africa, dtg_first=0" = "South Africa", 
        "ctry=South Africa, dtg_first=1" = "South Africa", 
        
        "ctry=Zambia, dtg_first=0" = "Zambia", 
        "ctry=Zambia, dtg_first=1" = "Zambia", 
        
         "ctry=Zimbabwe, dtg_first=0" = "Zimbabwe", 
        "ctry=Zimbabwe, dtg_first=1" = "Zimbabwe"
)))
...
ggsave("output/Cohort_initiation_outcomes/Fig4_Initiation_ctry_Cumul_incid_all_outcomes.png", width = 6, height =9)
```
...
fit1_ctry_dtg_sex <- survfit2(Surv(time, dtg_comp_b) ~ ctry +dtg_first + sex, data = FU_time, etype = dtg_first_txt_crr1b.factor)

ggcompetingrisks(fit = fit1_ctry_dtg_sex) + 
...
p20 <-ggcompetingrisks(fit = fit1_ctry_dtg_sex,
                 xlab = "Years since ART initiation", 
                  ylab = "Cumulative incidence (%)",
                 title = "(A) Initiated other ART                  (B) Initiated DTG ART",
                  subtitle ="Female                   Male                 Female                 Male") +
     scale_x_continuous(breaks = c(0, 365,  730, 1095), limits = c(0,1095), 
                        labels = c("0", "1", "2", "3")) +
  theme_ggsurvfit_default() +
 scale_fill_manual("",
 ...
 facet_wrap(~strata, ncol = 4, labeller = labeller(strata = 
      c("ctry=Lesotho, dtg_first=0, sex=Female" = "Lesotho", 
        "ctry=Lesotho, dtg_first=0, sex=Male  " = "", 
        "ctry=Lesotho, dtg_first=1, sex=Female" = "", 
        "ctry=Lesotho, dtg_first=1, sex=Male  " = "", 
        
   "ctry=Malawi, dtg_first=0, sex=Female" = "Malawi", 
        "ctry=Malawi, dtg_first=0, sex=Male  " = "", 
        "ctry=Malawi, dtg_first=1, sex=Female" = "", 
        "ctry=Malawi, dtg_first=1, sex=Male  " = "", 
       
 "ctry=Mozambique, dtg_first=0, sex=Female" = "Mozambique", 
        "ctry=Mozambique, dtg_first=0, sex=Male  " = "", 
        "ctry=Mozambique, dtg_first=1, sex=Female" = "", 
        "ctry=Mozambique, dtg_first=1, sex=Male  " = "", 
                
 "ctry=South Africa, dtg_first=0, sex=Female" = "South Africa", 
        "ctry=South Africa, dtg_first=0, sex=Male  " = "", 
        "ctry=South Africa, dtg_first=1, sex=Female" = "", 
        "ctry=South Africa, dtg_first=1, sex=Male  " = "", 
 
 "ctry=Zambia, dtg_first=0, sex=Female" = "Zambia", 
        "ctry=Zambia, dtg_first=0, sex=Male  " = "", 
        "ctry=Zambia, dtg_first=1, sex=Female" = "", 
        "ctry=Zambia, dtg_first=1, sex=Male  " = "", 
        
 "ctry=Zimbabwe, dtg_first=0, sex=Female" = "Zimbabwe", 
        "ctry=Zimbabwe, dtg_first=0, sex=Male  " = "", 
        "ctry=Zimbabwe, dtg_first=1, sex=Female" = "", 
        "ctry=Zimbabwe, dtg_first=1, sex=Male  " = ""
)))

#p20

p21 <- p20 + theme(
           plot.title = element_text(size = 14, hjust = 0), #center title
          plot.subtitle = element_text(size = 13, hjust = 0), #center subtitle
          axis.title.y = element_text(margin = margin(t = 0, r = 10, b = 0, l = 0), size = 12), #pad y axis label
          axis.title.x = element_text(margin = margin(t = 10, r = 0, b = 0, l = 0), size = 12), #pad x axis label
          axis.text.x = element_text(size = 10),
          axis.text.y = element_text(size = 10),
...
ggsave("output/Cohort_initiation_outcomes/Fig4_Initiation_ctry_sex_Cumul_incid_all_outcomes.png", width = 8, height =8)
```
